...
export default function Dashboard() {
  ...
  const filteredTransactions = myTransactions.filter(
    (tx) => new Date(tx.date) >= getStartDate()
  );
  ...
  const formatMoney = (amount?: number | null): string => {
    ...
    return Number(amount).toLocaleString() + " " + t("currency");
  };
  ...
  const getPeriodLabel = () => {
    ...
    switch (period) {
      case "month":
        return t("period.monthLabel", { month: now.getMonth() + 1, year: now.getFullYear() });
      case "6months":
        return t("period.sixMonthsLabel");
      case "year":
        return t("period.yearLabel", { year: now.getFullYear() });
      default:
        return "";
    }
  };
      ...
      <div className="rounded-2xl border bg-card shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-8 text-white">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold">{t("totalBalance")}</h2>
            </div>
            <button
          ...
          {loading ? (
            ...
            <div className="text-4xl font-extrabold tracking-tight">
              {hideBalance ? `${t("hiddenBalance")} ${t("currency")}` : `${totalBalance.toLocaleString()} ${t("currency")}`}
            </div>
          )}

          <p className="text-sm opacity-90 mt-3">
            {t("walletCount", { count: myWallets.length })}
          </p>
        </div>
        ...
        <div className="p-6 bg-muted/50">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold">
                        {isVisible
                          ? wallet.balance.toLocaleString() + " " + t("currency")
                          : `${t("hiddenBalance")} ${t("currency")}`}
                      </p>
                    </div>
                  </div>
                  <button
          ...
          <div className="mt-8 pt-6 border-t">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">{t("incomeExpense", { period: getPeriodLabel() })}</h3>
              <div className="flex gap-2 flex-wrap">
                {(["month", "6months", "year"] as const).map((key) => (
                  ...
                  >
                    {key === "month" ? t("period.thisMonth") : key === "6months" ? t("period.sixMonths") : t("period.thisYear")}
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="space-y-6">
        <div className="bg-card border rounded-2xl p-5 shadow-lg">
          <h3 className="text-lg font-semibold text-center mb-4">
            {t("expenseByCategory", { period: getPeriodLabel() })}
          </h3>
          ...
          {expenseByCategory.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground py-10">{t("noExpense")}</p>
          )}
        </div>
        ...
        <div className="bg-card border rounded-2xl p-5 shadow-lg">
          ...
          {myGoals.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground py-10">{t("noGoals")}</p>
          )}
        </div>
      </div>
    </section>
